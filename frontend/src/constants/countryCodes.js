// ITU-T E.164 international calling codes, one entry per distinct dial code
// (not one per country) — a handful of dial codes are shared by multiple
// countries/territories (NANP's +1, Russia/Kazakhstan's +7, the UK's +44
// covering Jersey/Guernsey/Isle of Man, etc.), and splitting those into
// separate rows with the same `code` would make a phone number's dial code
// impossible to round-trip back to a single dropdown selection. Keep it that
...
// spelled out in full. A closed <select> truncates from the right, so a
// long name pushes the dial code (at the end of the label) off screen right
// after selecting it; these are short enough that the code stays visible.
// They're left at their original alphabetical position (by full country
// name, e.g. 'US / Canada' still sits where "United States" would) rather
// than moved under "U" — reordering isn't worth the diff noise.
